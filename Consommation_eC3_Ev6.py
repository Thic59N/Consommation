...
def check_password():
    ...
    try:
        if "auth" in st.secrets:
            target_password = st.secrets["auth"].get("password", target_password)
    except:
        pass
...
with col_img:
    try:
        # On cherche maintenant le fichier .png pour la transparence
        img_path = os.path.join(os.path.dirname(__file__), "Kia ev6.png")
        if os.path.exists(img_path):
            image = Image.open(img_path)
            st.image(image, use_container_width=True)
        else:
            # Fallback si le PNG n'est pas trouvé
            st.write("🚗")
    except:
        st.write("🚗")
    ...
    if doc:
        if charge_en_cours:
            ...
            with st.form("form_debut"):
                ...
                km_actuel = st.number_input("Kilométrage actuel", value=int(km_precedent))
                p_dep = st.number_input("% Batterie départ", 0, 100)
